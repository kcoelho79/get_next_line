...
/*   Updated: 2021/06/18 16:34:33 by kde-oliv         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_next_line(int fd, char **line)
{
	...
	while (b_read > 0)
	{
		buff[b_read] = '\0';
		if (c_line[fd] == NULL)
			c_line[fd] = ft_strdup(buff);
		else
		{
			swp = ft_strjoin(c_line[fd], buff);
			free(c_line[fd]);
			c_line[fd] = swp;
		}
		if (ft_strchr(c_line[fd], '\n'))
			break ;
		b_read = read(fd, buff, BUFFER_SIZE);
	}
	free(buff);
	return (output(&c_line[fd], line, b_read));
}
